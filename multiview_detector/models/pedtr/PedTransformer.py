...
class PedTransformer(nn.Module): 
    def __init__(self, num_cams=7, num_query=200, embed_dims=512, num_points=1, img_reduced_size=4, grid_shape=[480, 1440], num_heads=4, dropout=0.1):
        ...
        self.query_gen = Query_genrator(num_query=self.num_query, channels=embed_dims)
        
        self.img_backbone  = nn.Sequential(*list(resnet18(pretrained=True,
                                                     replace_stride_with_dilation=[False, True, True]).children())[:-2])
        self.MLPs_query_to_ground_coordinates =  PreNorm(embed_dims, FeedForward(embed_dims, hidden_dim=128, output_dim=2))
        self.deformable_transformer = None 
        self.query = self.query_gen()


        self.multi_head_attn = nn.MultiheadAttention(embed_dim=embed_dims, num_heads=4, dropout=dropout, batch_first=True)
    ...
    def init_weight(self):
        ...
        nn.init.xavier_uniform_(self.output_proj.weight, gain=1) 
        nn.init.constant_(self.output_proj.bias, val=0)
    ...
    def feature_sampling(self, ground_coordinates, proj_mat, img_feats, num_cam, num_query, img_reduce_size, img_w, img_h):

        assert len(proj_mat.shape) == 4 and proj_mat.shape[1] == self.num_cams

        ground_coordinates = torch.cat((ground_coordinates, torch.ones(ground_coordinates.shape[0]).view(-1, 1)), dim=1)
        img_reduced_mat = torch.tensor([[1.0/img_reduce_size, 0., 0.],
                                       [0., 1.0/img_reduce_size, 0.], 
                                       [0., 0., 1.0]], dtype=torch.float32)
         
        img_coord = img_reduced_mat @ proj_mat[0].float() @ ground_coordinates.T # shape torch.Size([7, 3, 200])
        ...
        mask = mask.view(1, num_cam, 1, num_query, 1).permute(0, 2, 3, 1, 4)
        mask = torch.nan_to_num(mask)

        # feature sampling 
        BN, C, H, W = img_feats.size()
        reference_points_cam_lvl = reference_points_cam.view(BN, num_query, 1, 2)
        sampled_feat = F.grid_sample(img_feats, reference_points_cam_lvl, align_corners=False)
        sampled_feat = sampled_feat.view(1, C, num_query, num_cam, 1) # 1, 512, 200, N, 1
    ...
    def forward(self, img, ray=None, proj_mat=None):
        ...
        ground_coordinates = torch.sigmoid(self.MLPs_query_to_ground_coordinates(queries))
        ground_coordinates[:, 0] *=  self.grid_shape[0]
        ground_coordinates[:, 1] *=  self.grid_shape[1]
        
        reference_points_ground, output, mask = self.feature_sampling(ground_coordinates, proj_mat, img_features, self.num_cams, self.num_query, self.img_reduced_size, img_w=img_features.shape[-1], img_h=img_features.shape[-2])
        ...
        #update query 
        output_class = self.class_branch(output)
        output_coords = self.reg_branch(output)
